...
function formatColorRow(play: CFBGamePlay) {
    let classText = "";
    if (turnover_vec.includes(play.type.text) || (play.text.includes("fumble") && play.change_of_poss == 1) || (play.start.down == 4 && parseFloat(play.statYardage.toString()) < parseFloat(play.start.distance.toString()) && !play.type.text.includes('Punt') && !play.type.text.includes('Timeout') && !play.type.text.includes('Penalty'))) {
